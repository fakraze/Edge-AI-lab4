...
# Monkey-patch so every DDP wrapper enables find_unused_parameters

import torch.nn.parallel

# Grab the original class
_OrigDDP = torch.nn.parallel.DistributedDataParallel

# Subclass it to always set find_unused_parameters=True
class _DDP(_OrigDDP):
    def __init__(self, *args, **kwargs):
        kwargs.setdefault('find_unused_parameters', True)
        super().__init__(*args, **kwargs)

# Overwrite the name so Ultralytics picks up our subclass
...
def prune(args):
    ...
    # Only run the pruning loop on the master process (rank 0); 
    # worker ranks just handle the DDP training calls.
    ...
    val_batch = pruning_cfg.pop('val_batch', pruning_cfg['batch'])
    batch_size = pruning_cfg.get('batch', pruning_cfg['batch'])
    ...
    pruning_cfg['data'] = pruning_cfg['data'] if 'data' in pruning_cfg else 'coco128-seg.yaml'
    pruning_cfg['epochs'] = pruning_cfg['epochs'] if 'epochs' in pruning_cfg else 10
    ...
    # exit InferenceMode so we can re–enable grads
    ...
    # pruning_cfg['batch'] = pruning_cfg.get('val_batch', pruning_cfg['batch'])
    ...
    for i in range(args.iterative_steps):
        ...
        # run **everything** this iteration under enable_grad()
        with torch.enable_grad():
            model.model.train()
            # re‐enable grads on every parameter
            for name, param in model.model.named_parameters():
                param.requires_grad = True

            # 1) build and apply the pruner
            ignored_layers = []
            for m in model.model.modules():
                if isinstance(m, (Detect,)):
                    ignored_layers.append(m)
            pruner = tp.pruner.GroupNormPruner(
                model.model, example_inputs.to(model.device),
            ...
            # 2) pre‐fine‐tune validation
            pruning_cfg['name'] = f"step_{i}_pre_val"
            metric = deepcopy(model).val(**pruning_cfg)
            ...
            # 4) post‐fine‐tune validation
            pruning_cfg['name'] = f"step_{i}_post_val"
            pruning_cfg['batch'] = val_batch
            current_map = model.val(**pruning_cfg).box.map

            # 5) logging, graph, early‐stop checks …
            # (you already have that below)
        ...
        #       \n  mAP: {base_macs / 1e9: .5f} -> {pruned_map} \
        ...
        # fine-tuning — exit InferenceMode so we can re-enable grads
        with torch.enable_grad():
            for name, param in model.model.named_parameters():
                param.requires_grad = True
...
if __name__ == "__main__":
    # open log once, line-buffered (buffering=1)
    log_f = open('pruning.log', 'a', buffering=1)

    parser = argparse.ArgumentParser()
    parser.add_argument('--model', help='Pretrained pruning target model file')
    ...
    parser.add_argument("--device", default="0", help="CUDA device(s) to use, e.g. '0,1' for two GPUs")
